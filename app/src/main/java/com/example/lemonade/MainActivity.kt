...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val instructionText = TextView(this).apply {
            text = getString(R.string.prompt_select_lemon)
            textSize = 18f
            setTextColor(Color.BLACK)
            gravity = Gravity.CENTER
            setPadding(0, 0, 0, 16)
        ...
        val imageView = ImageView(this).apply {
            ...
            contentDescription = getString(R.string.desc_lemon_tree)
        ...
        imageView.setOnClickListener {
            when (currentStep) {
                1 -> {
                    ...
                    squeezeCount = 0
                    instructionText.text = getString(R.string.prompt_squeeze_lemon)
                    imageView.setImageResource(R.drawable.lemon_squeeze)
                    imageView.contentDescription = getString(R.string.desc_lemon)
                }
                2 -> {
                    squeezeCount++
                    if (squeezeCount >= requiredSqueezes) {
                        currentStep = 3
                        instructionText.text = getString(R.string.prompt_drink_lemonade)
                ...
                3 -> {
                    currentStep = 4
                    instructionText.text = getString(R.string.prompt_restart)
                    imageView.setImageResource(R.drawable.lemon_restart)
                    imageView.contentDescription = getString(R.string.desc_empty_glass)
                }
                4 -> {
                    currentStep = 1
                    instructionText.text = getString(R.string.prompt_select_lemon)
                    imageView.setImageResource(R.drawable.lemon_tree)
                    imageView.contentDescription = getString(R.string.desc_lemon_tree)
                }
                else -> {
                    Toast.makeText(this, getString(R.string.error_invalid_phase), Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
